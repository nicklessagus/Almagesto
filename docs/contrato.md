...
> 0–6. **Medición vigente (2026-08-24, tras las tandas 8 y 9, la auditoría triple y la pasada `/auditar`, más el fix de anclaje de secciones del 2026-08-25, sobre los 99)**:
> 77 *garantizados y medidos* · 2 *garantizados* (sin medición propia) · 13 *sin
> medir* · 6 *parciales* · 4 *HUECO* · 1 *INCUMPLIDO*. El salto no es trabajo de documentación: son
...
en las 48 categorías** salvo tres declaradas
...
| 5 | **Los siete `incomplete`: proxies de síntesis campo por campo.** `P_rot` sin documentar en la prosa, `activity_indicators_expected` vacío, planeta del frontmatter no discutido, paper core sin `methods`, paper extraído sin `role`, ficha sin ground-truth, ground-truth sin ficha. *(`thesis_links` sin `bearing` salió con D-21; el conteo es el de los sitios que pueblan `incomplete` en `lint.py`.)* | **Necesaria.** (B) derivó INV-45 (el paper llegó o no llegó); el sistema mide además **si la síntesis tocó cada eje**. Es la única red sobre la calidad de la capa LLM que no depende de otra corrida de LLM. |
