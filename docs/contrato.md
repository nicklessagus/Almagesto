...
> 0–6. **Medición vigente (2026-08-24, sobre los 91)**: 65 *garantizados y medidos* · 13 *sin medir*
> · 6 *parciales* · 6 *HUECO* · 1 *INCUMPLIDO*. El salto no es trabajo de documentación: son las
...
| **INV-41** | Cada clase de hallazgo tiene severidad fija, documentada y coherente con el daño. | P1 | **garantizado y medido** | El invariante mejor evidenciado del repo: mapeo **1:1 en ambas direcciones** entre las categorías del reporte y la prosa de `CLAUDE.md`, con la severidad correcta en cada caso (8ª F3(b)), y las bloqueantes del `n_block` coincidiendo con lo declarado (8ª F4 §2). ⚠ **Los números de esa medición caducaron**: eran 30 categorías y 12 bloqueantes; al 2026-08-24 son **41** (fijado por `tests/poblada/test_golden.py`) y **18** (19 con `--cierre`), y `CLAUDE.md` ya no las documentaba todas — se le agregaron las tres que faltaban. La re-medición del 1:1 queda pendiente. Ninguna categoría de la prosa nombra algo que ya no exista. |
