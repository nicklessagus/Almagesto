...
> 104 *garantizados y medidos* · 2 *garantizados* (sin medición propia) · 13 *sin
> medir* · 9 *parciales* · 5 *HUECO* · 1 *INCUMPLIDO*.
...
| **INV-134** | Una extracción declara **desde qué sujeto** se leyó el paper; el silencio de una nota sobre un eje no se puede leer como «se miró y no hay nada». | P1 | **parcial** (detector puesto; falta el productor) | Enunciado el 2026-08-27 (issue #188). El prompt nunca pregunta *«¿qué dice este paper?»* sino *«¿qué dice **sobre {name}**?»*, con los `grep` armados desde los alias de ese sujeto y los bullets ramificados por tipo (#76) — pero la nota es UNA por bibcode con una sola `## Extracción (LLM)` sin scope. Medido en una instancia real: **141 de 908** notas las reclaman 2+ sujetos y **ninguna** tiene una segunda extracción; `ingest-theme` lo produce por diseño en su paso 3b (retro-tag por grep, que corre DESPUÉS de la extracción). Es la misma familia que D-34 en las hipótesis (*«no hay evidencia» no es «no existe evidencia»*) y que la cobertura de `discover` (*corrió con N* vs *NO CORRIÓ*). Hoy: `cfg.load_vistas` / `cfg.load_no_vista` (forma dura, vocabulario cerrado `star|theme`, escotilla con motivo obligatorio) y cuatro categorías del lint — el schema viejo y la incoherencia `vistas[]`↔cuerpo **bloquean**; el reclamo no leído es backlog, y declarado con `no_vista`, informativo. **Lo que falta**: que la cadena las ESCRIBA (`make_notes` la sección, el prompt la vista, el cosechado el frontmatter — pasos 3–5 de #188). (`tests/test_lint.py::test_schema_viejo_sin_vistas_es_bloqueante`, `tests/test_lint.py::test_vista_declarada_sin_su_seccion_es_bloqueante`, `tests/test_lint.py::test_reclamo_sin_vista_es_backlog_y_nombra_al_sujeto`, `tests/test_lib_config.py::test_load_vistas_forma_canonica`) |
