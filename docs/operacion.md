...
## Apéndice A — el porqué largo de `CLAUDE.md` (AUD-475)

> `CLAUDE.md` se inyecta entero en cada sesión, así que desde AUD-475 lleva sólo **regla +
> consecuencia + ancla**. Lo que salió —el caso que produjo cada regla, las mediciones con sus
> números, la historia «hasta 1.x» y la mecánica fina— vive acá, **verbatim como estaba el
> 2026-09-22 (v1.307.1)** y con **los mismos encabezados** que `CLAUDE.md` (un nivel más abajo).
> Si esta versión larga y la corta divergen, **manda `CLAUDE.md` y el código**: esto es el porqué,
> no la regla vigente. Las secciones que no se recortaron (layout, arquitectura, append, query,
> contradicciones, mantenimiento, convenciones) no se repiten acá.

### A · Encabezado de `CLAUDE.md`
...
### A · Framework vs instancia — Regla de oro (no editar framework en la instancia)
...
### A · ⛔ Frontera dura — la bóveda es SÓLO bibliografía (regla #0, no negociable)
...
mecánica de una operación, a su skill. Se inyecta entero en cada sesión, y el ratchet
(`tools/doc-size-ratchet.yaml`) mide el nivel y no la pendiente —medido: 43 subas firmadas y 0
rechazadas en 14 días—, así que **toda suba declara qué sale, o por qué nada puede salir** (`sale:`).
...
##### A · Los cuatro roll-ups se ESTAMPAN, no son Dataview (D-10/D-11)
...
##### A · Disputas (`disputes`, a nivel nota, con posiciones explícitas — #71)
...
⛔ **El `bibtex` se TRAE de una exportación oficial, nunca se redacta (#397).** La ficha guardaba campos sueltos y no la **referencia**, así que el informe rearmaba la entrada de memoria — y una entrada inventada sale plausible sobre el dato que termina impreso. Lo baja `fetch_bibtex.py` (cierra la cadena) por la cascada **ADS → `doi.org` → arXiv**, y el cuarto caso —un libro, un manual— deja el campo **VACÍO**: un hueco es correcto, una cita inventada no. ⛔ **`venue` es la exportación del SITIO del venue (JMLR, NeurIPS, PMLR), canónica sin DOI ni arXiv (#484): la pega una persona, así que exige `bibtex_url`** y no se re-baja. El lint **bloquea** el `bibtex` sin `bibtex_source` (sin procedencia es un bloque escrito a mano) y reporta el drift frontmatter ↔ exportación.
...
##### A · La extracción es una lectura CON LENTE, y la nota declara cuál se hizo: `vistas[]` (#188)
...
##### A · Identidad: el `doi`/`arxiv_id`, no el bibcode (D-19)
...
##### A · `data_availability` — dónde están los DATOS del paper (#424)
...
##### A · Los dos artefactos y sus dos `*_source`
...
##### A · `role` — qué TIPO de aporte es el paper (#73)
...
##### A · El aviso de capa LLM y las cuatro secciones de lectura (#124, #247)
...
##### A · Notas off-ADS y fuentes largas
...
#### A · Estándar transversal de autosuficiencia
...
#### A · Setup (definir el objetivo — paso 0, skill `setup`)
...
#### A · Relevancia de un TEMA DE MÉTODO — la lente propia y las tres puertas (D-26 / INV-88)
...
#### A · Descubrimiento multi-backend y anclado (`scripts/discover.py`, #104)
...
#### A · Ingest (una fuente → cascada de páginas)
...
##### A · El CICLO DE LA LENTE — cómo se encadenan las piezas (#310)
...
#### A · Registro de ingesta (`vault/config/registro/<slug>.yaml` — versionado, #51/#64)
...
#### A · Los cuatro cuadrantes de la curación — quién decidió y por qué (#111)
...
(propone y no escribe). Las otras tres salidas suponen que el equivocado es el repo y pedían publicar
una **atribución falsa** (`2012Naik`: Crossref perdió «Naik»). ⛔ **El drift `bibtex` ↔ frontmatter
(#397) lleva la MISMA firma, en la nota** (#483), vía `fetch_bibtex.py --paper <bib> --firmar
--campo <c> --motivo`. ⛔ **Cubre un ESTADO, no un campo:** vale mientras `declarado` y `catalogo`
sean los que firmó —si el catálogo se corrige, vuelve— y la firma vieja o rota **no se ignora** (D-4, #71).
...
#### A · Verify (chequeo claim↔fuente — skill `verify-citations`)
...
#### A · Auditoría de una FICHA (skill `audit-note`)
...
#### A · Propuestas (lo que el sistema sugiere y espera una firma — `scripts/proposals.py`)
...
### A · Reglas de método 4-6 (las de operar la bóveda)
