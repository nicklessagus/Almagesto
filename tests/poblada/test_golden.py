...
def test_reporte_lista_todas_las_categorias(sembrar):
    ...
    de `--cierre`), y desde el 2.1, **34** (el detector del registro pre-D-28). Esa categoría es además la ÚNICA
    ...
    **nombrada** ahí. O sea que la sección no desaparece en silencio, que es lo que este test
    ...
    assert len(titulos) == 34, (
        f"el reporte trae {len(titulos)} categorías, se esperaban 34 — alguna sección dejó de "
