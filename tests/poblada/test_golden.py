...
def test_reporte_lista_todas_las_categorias(sembrar):
    ...
    y desde el 3.1/3.3, **37** (lista de papers
    desactualizada, recorte de lectura sin declarar). Esa categoría es además la ÚNICA
    ...
    **nombrada** ahí. O sea que la sección no desaparece en silencio, que es lo que este test
    ...
    assert len(titulos) == 37, (
        f"el reporte trae {len(titulos)} categorías, se esperaban 37 — alguna sección dejó de "
