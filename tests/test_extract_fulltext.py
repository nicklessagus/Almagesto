...
def test_ecuaciones_vacias_se_detectan():
    si, why = ef.symbols_lost(VACIAS)
    assert si is True
    assert "SIN cuerpo" in why


def test_ecuaciones_con_cuerpo_no_se_marcan():
    assert ef.symbols_lost(VIVAS)[0] is False


def test_pocas_ecuaciones_es_NO_EVALUADO_no_ok():
    """D-43: un `0` que nadie midió no es un veredicto. 275 de 813 .txt del corpus de calibración
    caen acá (34%), así que devolver False sería un falso limpio a gran escala."""
    si, why = ef.symbols_lost("x = As    (1)\ns = Wx    (2)\n")
    assert si is None
    assert "no alcanza para medir" in why


def test_enumeracion_en_prosa_no_es_ecuacion():
    """Falso positivo real medido en 2004Himberg: sus `(1)`-`(5)` son una lista enumerada, y una
    versión anterior del detector lo marcaba al 100% con las ecuaciones intactas."""
    lista = "\n".join(f"({i}) Parameters for the ICA estimation are chosen" for i in range(1, 6))
    assert ef.symbols_lost(lista)[0] is None       # ni siquiera hay marcadores que contar


def test_referencia_en_prosa_no_cuenta_como_ecuacion():
    prosa = "\n".join("as shown in (5), the estimator is consistent" for _ in range(6))
    assert ef.symbols_lost(prosa)[0] is None


def test_numero_alineado_a_la_derecha_no_es_salto_de_columna():
    """En un paper de UNA columna el nº de ecuación va separado del cuerpo por una tira larga de
    espacios — idéntica a la canaleta entre columnas. Partir ahí hacía que el .txt más limpio del
    corpus diera 87% perdidas (medido)."""
    una_columna = "\n".join(f"kurt(v) = E{{v4}} - 3(E{{v2}})2 .{' ' * 30}({i}.1)" for i in range(1, 6))
    assert ef.symbols_lost(una_columna)[0] is False


def test_symbols_header_lo_puede_leer_make_notes():
    why = ef.symbols_lost(VACIAS)[1]
    assert ef.symbols_header(why).startswith(ef.SYMBOLS_MARK)
    assert "abrir el PDF" in ef.symbols_header(why)


# El backfill exige que el `.txt` sea LEGIBLE antes de mirarlo (el ilegible es del otro carril),
# así que la fixture necesita cuerpo real además de los marcadores vacíos.
VACIAS_LEGIBLE = LIMPIO + "\n" + VACIAS


def test_backfill_de_simbolos_marca_el_txt_ya_extraido():
    """Gemelo de `backfill_scanned_mark` para #113: el chequeo sólo corría sobre texto recién
    extraído, así que un `.txt` viejo se quedaba mudo. Medido: los dos casos del corpus de ICA."""
    why = ef.backfill_symbols_mark(VACIAS_LEGIBLE)
    assert why is not None
    assert "SIN cuerpo" in why


def test_backfill_de_simbolos_no_toca_un_txt_con_ecuaciones():
    assert ef.backfill_symbols_mark(VIVAS) is None


def test_backfill_de_simbolos_es_idempotente():
    ya = ef.symbols_header("motivo previo") + VACIAS_LEGIBLE
    assert ef.backfill_symbols_mark(ya) is None


def test_backfill_de_simbolos_no_pisa_el_carril_del_garble():
    """Un `.txt` que ya avisa que es OCR del editor ya le dice al lector que abra el PDF ante una
    duda de símbolos; apilar los dos headers sería ruido."""
    ya_ocr = ef.scanned_header("garble previo") + VACIAS_LEGIBLE
    assert ef.backfill_symbols_mark(ya_ocr) is None


def test_backfill_de_simbolos_no_toca_un_txt_ilegible():
    assert ef.backfill_symbols_mark("\x0c \x0c " * 50) is None
