...
5. **Cierre estándar** (idéntico a ingest): **auto-revisión de autosuficiencia** de la nota destino
   ...
   **verificación stale** si quedó atrás (#56)— → `python scripts/lint.py --cierre <slug>` en 0 (R-1: en el cierre un par de verificación vencido frena; sin el flag sólo reportaría. #121: la deuda de otro sujeto se reporta y no frena) → bookkeeping (`vault/wiki/log.md` SIEMPRE —
   entrada `append: <fuente> → <destino>`; `vault/wiki/index.md` y `vault/STATUS.md` sólo si cambió
   algo catalogable/de estado; `triage.py <slug> --sintesis` + `make_notes.py <slug>`, #523) → `git add` de archivos específicos + commit → **preguntar antes de
   `push`**.
