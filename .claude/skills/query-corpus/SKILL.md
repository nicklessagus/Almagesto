...
1. **Mirar el índice y el frontmatter** primero: `wiki/index.md`, las fichas `wiki/stars/*.md`
   (frontmatter máquina-legible) y `raw/ground_truth/*.json` suelen tener la respuesta directa
   (P_rot, planetas, indicadores, métodos aplicados).

2. **Si hace falta el texto**, grep sobre el texto completo local:
   ```bash
   grep -ril "<términos>" raw/fulltext/                # qué papers
   grep -in  "<términos>" raw/fulltext/<slug>/<bib>.txt # contexto en uno
   ```
...
4. **Archivar si vale la pena re-preguntarlo** (sobre todo si el pipeline va a cambiar): guardar en
   `wiki/queries/<slug>.md`. Si es descartable, basta la respuesta en el chat. Actualizar
   `wiki/log.md` si se archivó.

5. **Verificar citas (si se archivó en `wiki/`)**: correr el skill `verify-citations` sobre la nota
...
6. **Chequeo de salud (si se escribió en `wiki/`)**: correr `python scripts/lint.py` antes de
...
7. **Cierre (commit + push).** **Solo si se archivó algo en `wiki/`** (si la respuesta quedó solo en
...
"filearlo" de vuelta en `wiki/` para que la exploración sume. Declarar agregados (mean/median)
