...
def backfill_scanned_mark(prev: str) -> str | None:
    """Motivo si hay que estampar `source: ocr` sobre un `.txt` ya escrito, `None` si no.

    El chequeo de garble sólo corría al extraer, así que un `.txt` escrito antes se quedaba
    `pdftotext` para siempre — el camino de skip lo re-leía sólo para preguntarle si era ILEGIBLE, y
    un escaneo del editor es perfectamente legible. Medido sobre una bóveda real: 3 de los 42 `.txt`
    de un tema, y uno de ellos un paper cuya ecuación la nota había transcrito con un subíndice
    equivocado por el OCR.

    No re-extrae: para un escaneo del editor la capa de texto del PDF ya es el mejor texto que hay
    (ver `scanned_header`), así que es un estampado de header — e idempotente, porque un `.txt` que
    ya lleva la marca no se vuelve a puntuar.
    """
    # @inv INV-28
...
def main() -> int:
    ...
    if stamped:
        print(f"  notas: {stamped} con fulltext:/fulltext_source:/pdf_source: estampados "
              "(contrato máquina)")
    if not failed:
        cfg.save_paso(args.slug, "extract_fulltext", flags=cfg.flags_usados(args, ap))
