...
def main() -> int:
    ...
    # #331 — el guard del sujeto va ANTES de la línea de arranque (ver `cfg.subject_refusal`).
    if (motivo := cfg.subject_refusal(args.slug, "theme" if args.theme else "star",
                                      "no se generó ninguna nota",
                                      f"Corré: `{cfg.make_notes_cmd(args.slug)}`")) is not None:
